...
function animateSkillBar(containerSelector, endValue, speed) {
   const skillbarLevel = document.querySelector(
      `${containerSelector} .skillbar-circle`
   );
   const skillbarValue = document.querySelector(
      `${containerSelector} .circle-value`
   );
   ...
   const level = setInterval(function () {
      ...
      skillbarLevel.style.background = `conic-gradient(#493C9F, #7865CE, #A890FE, #111772 ${
         startValue * 3.6
      }deg, #ededed 0deg)`;
...
function fillTitles() {
   for (let i = 0; i < projects.length; i++) {
      document.querySelector(".title" + (i + 1)).textContent =
         projects[i].title;
      document.querySelector(".customer" + (i + 1)).textContent =
         projects[i].customer;
